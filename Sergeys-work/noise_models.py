...
def generate_synthetic_gt(velocity_px_s: float, obj_size_px: float,
                           bg_density: float = 0.05,
                           num_objects: int = 5,
                           num_frames: int = 300,
                           fps: float = 30.0,
    ...
    for obj_id in range(1, num_objects + 1):
        start_y = rng.uniform(obj_size_px, scene_height - obj_size_px * 2)
        start_x = rng.uniform(0, scene_width * 0.2)
        y_drift = rng.uniform(-0.05, 0.05) * velocity_px_s
        for frame_num in range(1, num_frames + 1):
            t = (frame_num - 1) * dt
            current_x = (start_x + velocity_px_s * t) % (
                scene_width + obj_size_px)
            current_y = np.clip(start_y + y_drift * t,
                                 0, scene_height - obj_size_px * 2)
            rows.append({
                'frame': frame_num, 'id': obj_id,

...
def _run_one_sweep_b(config: dict) -> dict:
    ...
    if source == 'mot17':
        ...
        gt_sub = generate_synthetic_gt(
            velocity_px_s = synth['velocity_px_s'],
            obj_size_px   = synth['obj_size_px'],
            bg_density    = synth['bg_density'],
            num_objects   = synth.get('num_objects', 5),
            num_frames    = config['max_frames'],
            fps           = synth.get('fps', 30.0),
            seed          = int(config['seed']) * 17 + 3,
        )
        fps = synth.get('fps', 30.0)
    ...
    if config['sensor_type'] == 'DVS':
        ...
        dets = simulate_dvs_noisy_gt(gt_sub, fps, cfg, rng)
    else:
...
SYNTHETIC_SCENES = [
    {'label': 'synthetic_low',  'velocity_px_s': 200, 'obj_size_px': 50,
     'bg_density': 0.05, 'num_objects': 5, 'fps': 30.0},
    {'label': 'synthetic_high', 'velocity_px_s': 200, 'obj_size_px': 50,
     'bg_density': 0.40, 'num_objects': 5, 'fps': 30.0},
]


def _build_one_source(source_label: str, synth_scene: dict | None,
                       max_frames: int, num_seeds: int,
                       fp_calibration: dict,
    ...
    coast_values = [False, True] if include_coast_on else [False]

    for dvs in DVS_SENSORS:
        for theta in dvs_thetas:
            for coast in coast_values:
...
def _build_sweep_b_configs(max_frames: int, num_seeds: int,
    ...
    for scene in SYNTHETIC_SCENES:
        configs += _build_one_source(
            ...
            fp_calibration, include_coast_on=False,
        )
    return configs

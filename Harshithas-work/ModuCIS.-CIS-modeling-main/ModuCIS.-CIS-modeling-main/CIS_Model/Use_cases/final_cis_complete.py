...
#
# PART 1 — CIS analysis (static image scene from Ramaa)
#   - Sweeps all object sizes x velocities x backgrounds
#   - Saves all plots + 5 CSVs
#
# PART 2 — Camera vs CIS animation
#   - Left  = camera view (smooth sphere)
#   - Right = CIS snapshot view (sphere freezes between captures)
#   - Bottom = live event rate graph, CIS power flat line
#   - Both backgrounds shown (low + high texture)
#   - Shows interactive window AND saves GIF/MP4
...
try:
    from visualcomputing import (
        object_sizes, velocities, backgrounds,
        safety_factor, false_pos_rate,
    ...
    print("[OK] Imported Ramaa's scene model")
except ImportError as e:
    raise ImportError(f"visualcomputing.py not found in {HERE}\nError: {e}")
except Exception as e:
    print(f"[WARN] Partial import: {e} — using fallbacks")
    try:
        from visualcomputing import (compute_fps_min, compute_min_snr,
    ...
    import matplotlib.patches as patches
    from matplotlib.patches import Circle
    object_sizes=[25,50,100]; velocities=[10,50,100,200,500]
...
# ── Animation parameters — read from Ramaa's file ─────────────────────────
# If she changes these, your code automatically updates
try:
    from visualcomputing import velocities_viz as VELOCITIES_ANIM
    print(f"[OK] Animation velocities from Ramaa: {VELOCITIES_ANIM}")
except Exception:
    VELOCITIES_ANIM = [50, 500, 2000]
    print(f"[WARN] velocities_viz not found — using fallback {VELOCITIES_ANIM}")

try:
    from visualcomputing import ANIM_BG
    print(f"[OK] Animation background from Ramaa: {ANIM_BG}")
except Exception:
    ANIM_BG = list(backgrounds.keys())[0]
    print(f"[WARN] ANIM_BG not found — using fallback: {ANIM_BG}")

try:
    from visualcomputing import ANIM_OBJ_SIZE as ANIM_OBJ_SIZE_PX
    print(f"[OK] Animation object size from Ramaa: {ANIM_OBJ_SIZE_PX}px")
except Exception:
    ANIM_OBJ_SIZE_PX = 50
    print(f"[WARN] ANIM_OBJ_SIZE not found — using fallback: {ANIM_OBJ_SIZE_PX}px")

# ── Velocity ranges ────────────────────────────────────────────────────────
VELOCITIES_STATIC = sorted(set(list(velocities) + [1000, 2000]))

# ── Animation constants ────────────────────────────────────────────────────
ANIM_R     = 30      # sphere radius px
ANIM_Y     = 185     # sphere vertical centre
ANIM_X0    = 60      # sphere start x
ANIM_STEPS = 60      # frames per velocity phase

def _positions(vel, n=ANIM_STEPS):
    """Sphere x positions across n frames — faster vel = bigger jumps."""
    ppf = max(4, min(scene_width * 0.8 / n, vel / VELOCITIES_ANIM[0] * 6))
    return [min(ANIM_X0 + i * ppf, scene_width - ANIM_R - 10)
            for i in range(n)]
...
def run_cis_instance(adc_bits):
    """Run CIS_Array once at WORST_CASE_FPS. Power is constant."""
    buf = io.StringIO()
...
def make_row(bg_name, obj_px, vel, scene_tag):
    ...
    return {
        ...
        'event_rate_ev_s':  compute_event_rate(vel, obj_px, bg_dens, false_pos_rate),
    }

# Static image scene rows
static_rows = [make_row(bg, obj, vel, 'static_image')
               for bg in backgrounds
               for obj, vel in product(sorted(object_sizes), VELOCITIES_STATIC)]

# Animation scene rows — Ramaa's exact velocities, background, object size
...
save_csv(anim_rows,   'cis_animation_scene_summary.csv')
save_csv([
...
# ══════════════════════════════════════════════════════════════════════════
#  PART 1 PLOTS — Static image scene
# ══════════════════════════════════════════════════════════════════════════
...
# A1 Power vs velocity
fig, axes = plt.subplots(1,2,figsize=(14,5))
fig.suptitle('CIS Power vs Velocity  [Static Image Scene]\n'
             'Constant — locked to worst-case FPS', fontweight='bold')
for ax, bg in zip(axes, backgrounds):
    pc = bg_cache[bg][0]
    for obj in sizes_s:
        sub = df_s[(df_s.background==bg)&(df_s.object_size_px==obj)].sort_values('velocity_px_s')
        ax.plot(sub.velocity_px_s, sub.power_mW,
                color=OBJ_COLORS[obj], lw=2.5, marker='o', label=f'obj={obj}px')
    ax.axhline(y=pc, color='red', ls='--', lw=1.5, label=f'Fixed ({pc:.1f} mW)')
    ax.set(title=f'Background: {bg}', xlabel='Velocity (px/s)', ylabel='CIS Power (mW)')
    ax.set_xscale('log'); ax.set_ylim(0, pc*1.3)
    ax.legend(fontsize=8); ax.grid(True, alpha=0.4)
plt.tight_layout(); save_fig('cis_power_vs_velocity.png')

# A2 Event rate vs power
fig, axes = plt.subplots(1,2,figsize=(14,5))
fig.suptitle('CIS Power vs Scene Event Rate  [Static Image Scene]\n'
             'Power constant regardless of scene activity', fontweight='bold')
for ax, bg in zip(axes, backgrounds):
    pc = bg_cache[bg][0]
    for obj in sizes_s:
        sub = df_s[(df_s.background==bg)&(df_s.object_size_px==obj)].sort_values('event_rate_ev_s')
        ax.plot(sub.event_rate_ev_s, [pc]*len(sub),
                color=OBJ_COLORS[obj], lw=2.5, marker='o', label=f'obj={obj}px ({pc:.1f} mW)')
    ax.set(title=f'Background: {bg}',
           xlabel='Scene Event Rate (events/s)', ylabel='CIS Power (mW)  [constant]')
    ax.set_xscale('log'); ax.legend(fontsize=7); ax.grid(True, alpha=0.4)
    ax.annotate('CIS power does NOT\nchange with scene activity',
                xy=(0.05,0.85), xycoords='axes fraction', fontsize=9, color='darkred',
                bbox=dict(boxstyle='round', facecolor='#fff0f0', edgecolor='red'))
plt.tight_layout(); save_fig('cis_eventrate_vs_power.png')

# A3 Latency vs velocity
fig, axes = plt.subplots(1,2,figsize=(14,5))
fig.suptitle('CIS Latency vs Velocity  [Static Image Scene]\n'
...
for ax, bg in zip(axes, backgrounds):
    ...
    for obj in sizes_s:
        ...
        ax.plot(sub.velocity_px_s, sub.latency_ms,
                color=OBJ_COLORS[obj], lw=2.5, marker='^', label=f'obj={obj}px')
    ax.axhline(y=lat, color='purple', ls='--', lw=1.5, label=f'Frame time ({lat:.3f} ms)')
    ax.set(title=f'Background: {bg}', xlabel='Velocity (px/s)', ylabel='CIS Latency (ms)')
    ax.set_xscale('log'); ax.legend(fontsize=8); ax.grid(True, alpha=0.4)
plt.tight_layout(); save_fig('cis_latency_vs_velocity.png')

# A4 SNR and dynamic range
fig, axes = plt.subplots(1,2,figsize=(14,5))
fig.suptitle('CIS SNR and Dynamic Range vs Velocity', fontweight='bold')
ax_snr, ax_dr = axes
for bg in backgrounds:
    sub = df_s[(df_s.background==bg)&(df_s.object_size_px==50)].sort_values('velocity_px_s')
...
ax_snr.set(title='SNR vs Velocity (obj=50px)', xlabel='Velocity (px/s)', ylabel='SNR (dB)')
ax_snr.set_xscale('log'); ax_snr.legend(); ax_snr.grid(True, alpha=0.4)
for bg in backgrounds:
    dr = bg_cache[bg][1].get('dynamic_range_dB')
    if dr:
        sub = df_s[(df_s.background==bg)&(df_s.object_size_px==50)].sort_values('velocity_px_s')
        ax_dr.plot(sub.velocity_px_s, [dr]*len(sub),
                   color=BG_COLORS[bg], marker='s', lw=2, label=f'{bg} ({dr:.1f} dB)')
ax_dr.set(title='Dynamic Range vs Velocity (constant)',
          xlabel='Velocity (px/s)', ylabel='DR (dB)')
ax_dr.set_xscale('log'); ax_dr.legend(); ax_dr.grid(True, alpha=0.4)
plt.tight_layout(); save_fig('cis_snr_dr_vs_velocity.png')

# A5 & A6 Heatmaps per background
for bg in backgrounds:
    sub_bg = df_s[df_s.background==bg]
    P = np.full((len(sizes_s), len(VELOCITIES_STATIC)), np.nan)
    F = np.zeros_like(P, dtype=int)
    for i,s in enumerate(sizes_s):
        for j,v in enumerate(VELOCITIES_STATIC):
    ...
    for grid, cmap, label, tag in [
        (P, 'viridis', 'CIS Power (mW)\nUniform = constant power', 'power_heatmap'),
        (F, 'Greens',  'Feasible (1=yes 0=no)',                    'feasibility_heatmap')
    ]:
        fig, ax = plt.subplots(figsize=(10,4))
        im = ax.imshow(grid, aspect='auto', cmap=cmap, origin='lower',
                       interpolation='nearest',
                       **({'vmin':0,'vmax':1} if tag=='feasibility_heatmap' else {}))
        plt.colorbar(im, ax=ax, label=label)
        ax.set_xticks(range(len(VELOCITIES_STATIC)))
        ax.set_xticklabels(VELOCITIES_STATIC, rotation=45, fontsize=8)
        ax.set_yticks(range(len(sizes_s))); ax.set_yticklabels(sizes_s)
        ax.set(xlabel='Velocity (px/s)', ylabel='Object Size (px)',
               title=f'CIS {tag.replace("_"," ").title()} — {bg}')
        plt.tight_layout()
        save_fig(f'cis_{tag}_{bg}.png')

# ══════════════════════════════════════════════════════════════════════════
#  PART 2 — Camera vs CIS snapshot animation
#  Both backgrounds (low + high texture) shown
#  Bottom panel = live event rate graph with flat CIS power line
# ══════════════════════════════════════════════════════════════════════════
matplotlib.use('TkAgg')
import matplotlib.pyplot as plt
from matplotlib.animation import FuncAnimation

# Build full frame list across all 3 velocity phases
anim_frames = []
for vel in VELOCITIES_ANIM:
    fps_min_v    = compute_fps_min(vel, ANIM_R*2, safety_factor)
    xs           = _positions(vel)
    fps_scaled   = max(1, min(ANIM_STEPS, int(fps_min_v/VELOCITIES_ANIM[0]*3)))
    cis_interval = max(1, ANIM_STEPS//fps_scaled)
    cis_x        = xs[0]
    for i, x in enumerate(xs):
        if i % cis_interval == 0: cis_x = x
        anim_frames.append((vel, x, cis_x, fps_min_v, i, cis_interval))
...
def make_sphere_set(ax):
    ...
    trl = [Circle((ANIM_X0,ANIM_Y), ANIM_R, color="#4488DD",
                   alpha=0.12*(i+1), zorder=3) for i in range(3)]
...
all_evt_low  = [compute_event_rate(vel, ANIM_OBJ_SIZE_PX,
                backgrounds['low_texture'],  false_pos_rate) for vel,*_ in anim_frames]
all_evt_high = [compute_event_rate(vel, ANIM_OBJ_SIZE_PX,
                backgrounds['high_texture'], false_pos_rate) for vel,*_ in anim_frames]
...
ax_live.set_xlabel(f'Animation Frame  ({" → ".join(str(v) for v in VELOCITIES_ANIM)} px/s)',
...
def _update(fi):
    vel, xc, xk, fps_min_v, pi, ci = anim_frames[fi]
    ...
    def move_cis(sph, shi, shd, fl, txt, fps_txt):
        ...
        txt.set_text(f"vel = {vel} px/s")
        fps_txt.set_text(f"min FPS={fps_min_v:.0f} | every ~{ci} frames")
...
# Save to sweeps_results folder
try:
    from matplotlib.animation import FFMpegWriter
    mp4 = os.path.join(OUT_DIR, 'cis_camera_vs_snapshot_animation.mp4')
    anim2.save(mp4, writer=FFMpegWriter(fps=16, bitrate=2000),
               savefig_kwargs={'facecolor':'#F0F0F0'})
    print(f'[Saved] {mp4}')
except Exception:
    gif = os.path.join(OUT_DIR, 'cis_camera_vs_snapshot_animation.gif')
...
# ══════════════════════════════════════════════════════════════════════════
#  PART 2 PLOT — Static event rate vs CIS power for animation velocities
# ══════════════════════════════════════════════════════════════════════════
matplotlib.use('Agg')
import matplotlib.pyplot as plt
...
fig3.suptitle('CIS Power vs Scene Event Rate  [Animation Scene]\n'
...
for ax, bg in zip(axes3, backgrounds):
    ...
    sub = df_a[df_a.background==bg].sort_values('event_rate_ev_s') \
          if bg in df_a.background.values else pd.DataFrame()
    if sub.empty:
        ax.set_title(f'Background: {bg} (no data)')
        continue
    ax.plot(sub.event_rate_ev_s, [pc]*len(sub),
    ...
    ax.annotate('CIS power does NOT change\nwith scene event rate',
                ...
                bbox=dict(boxstyle='round', facecolor='#fff0f0', edgecolor='red'))
plt.tight_layout()
save_fig('cis_animation_eventrate_vs_power.png')
...
print('        cis_animation_scene_summary.csv')
...
print('        cis_animation_eventrate_vs_power.png')
print('  ANIM: cis_camera_vs_snapshot_animation.gif / .mp4')

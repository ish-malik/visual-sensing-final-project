...
> **Download the `cis_ready_to_run/` folder** from this repo — it contains the complete ModuCIS base plus all contributed scripts ready to execute.
 
**Folder structure inside `cis_ready_to_run/`:**
```
cis_ready_to_run/
├── [ModuCIS base files]        ← APS.py, CDS.py, Noise.py etc (professor's tool)
└── CIS_Model/
    └── Use_cases/
        ├── final_cis_complete1.py      ← synthetic sweep · run this first
        └── CIS_real_video/
            ├── cis_real_video.py       ← MOT17 noise characterization
            ├── cis_noise_tracking.py   ← clean vs noisy tracking
            ├── cis_pixel_comparison.py ← pixel-level noise figures
...
```bash
cd cis_ready_to_run/CIS_Model/Use_cases/
...
```bash
cd cis_ready_to_run/CIS_Model/Use_cases/CIS_real_video/

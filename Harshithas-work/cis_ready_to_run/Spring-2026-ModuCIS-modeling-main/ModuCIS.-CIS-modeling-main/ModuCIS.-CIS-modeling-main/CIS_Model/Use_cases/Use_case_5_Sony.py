...
PIXEL_BINNING = [4, 4]
INPUT_CLK_FREQ = 100e6  # 100 MHz (note: 1e5 is 100 kHz, you might mean 1e8)
PGA_DC_GAIN = 8
NUM_MUX_INPUT = 0
NUM_SUBFRAMES = 1
INPUT_PIXEL_MAP  = [
...
sensor = CIS_Array(
    ...
    num_PD_per_tap=8,  # Number of photodiodes per tap (1 by default; multi-PD designs use multiple PDs)
    ...
    PD_tech=0,  # PD technology index (not currently used, kept for future reference)
    feature_size_nm=45,  # Process technology node in nm
    ...
    analog_V_dd=1.8,  # Analog supply voltage in V
    digital_V_dd=1.0,  # Digital supply voltage in V
    bias_voltage=0.55,  # Bias voltage in V (used to bias source follower of the pixel)
    comparator_bias_voltage=0.55,  # Comparator bias voltage in V (used to bias comparators in ADC, CDS, PGA, etc.)
    ramp_bias_voltage=0.55,  # Ramp generator bias voltage in V
